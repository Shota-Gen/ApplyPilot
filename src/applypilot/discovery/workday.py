...
def store_results(conn: sqlite3.Connection, jobs: list[dict], employers: dict) -> tuple[int, int]:
    ...
    for job in jobs:
        ...
        try:
            conn.execute(
                "INSERT INTO jobs (url, title, salary, description, location, site, source, strategy, "
                ...
                (url, job.get("title"), None, short_desc, job.get("location"),
                 site, "workday", strategy, now, full_description, url, detail_scraped_at, detail_error),
            )
            new += 1
